...
class UiWeatherMapper @Inject constructor(
    ...
    override fun map(source: Observable<Result<LocationWeatherDto>>): Observable<Result<UiWeather>> {
        return source.mapResult {
            UiWeather(
                it.name,
                it.timeZone,
                map24HourFormat(it.timeFormat),
                map12HourFormat(it.timeFormat),
    ...
    private fun map24HourFormat(timeFormat: TimeFormatDto): String? {
    ...
    private fun map12HourFormat(timeFormat: TimeFormatDto): String? {
        return when(timeFormat) {
